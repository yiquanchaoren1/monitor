...
from dmxperf.analysis.network_plotter import NetworkPlotter
...
class PerfController:
    ...
    def _run_single_job(self, job, index):
        ...
        case_type = job.get('case_type', 'ivc').lower()
        print(f"🚀 [Controller] Job: {case_name} (Type: {case_type}) (Run {index})")
        
        runner = TaskRunner(self.global_cfg, self.run_root, self.dry_run)
        nodes_list = []

        # 1. 定义硬件任务黑名单
        HARDWARE_TASKS = ['hardware']
        # 2. 只要不在黑名单里，就启动监控
        need_monitor = case_type not in HARDWARE_TASKS
        ...
        try:
            ...
            # 对于硬件任务，节点列表可能很长，只打印前几个
            display_nodes = str(nodes_list)
            if len(nodes_list) > 5:
                display_nodes = f"['{nodes_list[0]}', ..., '{nodes_list[-1]}'] ({len(nodes_list)} nodes)"

            print(f"│   ├── Workload: {case_type.upper()}")
            print(f"│   └── Nodes: {display_nodes}")

            # === 2. Monitor Start (仅在需要时显示) ===
            if need_monitor:
                print(f"├── 👁️  [Monitor] Start")
                interval = job.get('interval', self.global_cfg.get('interval', 1.0))
                gpus_per_proc = job.get('gpus_per_proc', 1) 
                if 'node_layout' in job and isinstance(job['node_layout'][0], dict):
            ...
            # === 5. Analysis (仅在需要时显示) ===
            # 硬件任务不需要 Analysis 阶段的输出了，日志已由 TaskRunner 打印在 exec 中
            if need_monitor:
                ...
                if not self.dry_run:
                    ...
                    if job.get('visualize'): 
                        ...
                        if "network" in job.get('visualize', []):
