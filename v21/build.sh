...
if [ ! -f "dmxperf/src/cpp/multi_gpu_gemm.cpp" ]; then
    echo "❌ Error: 'multi_gpu_gemm.cpp' missing in dmxperf/src/cpp!"
    echo "   👉 请先将文件移动到该目录: mv multi_gpu_gemm.cpp dmxperf/src/cpp/"
    exit 1
fi
...
echo "   ./release/bin/dmxperf -c task.json   (监控)"